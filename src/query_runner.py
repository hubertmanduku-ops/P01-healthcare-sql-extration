...
class SQLQueryRunner:
    ...
    def demo_basics(self) -> None:
        ...
        demos = [
            ...
            ("Doctors earning over £300k",
             f"SELECT first_name, last_name, specialization, salary FROM {self.industry}.doctors WHERE salary > 300000 ORDER BY salary DESC LIMIT 10"),

            ("Active doctors in Radiology or Orthopedics",
             f"SELECT first_name, salary FROM {self.industry}.doctors WHERE specialization IN ('Radiology', 'Orthopedics') AND is_active = TRUE ORDER BY salary DESC LIMIT 10"),
        ]
    ...
    def demo_aggregation(self) -> None:
        ...
        """
        print("\n── Department/Specialization Salary Summary:")
        df = self.run(sql)
        if not df.empty:
            print(df.to_string(index=False))
    ...
    def demo_joins(self) -> None:
        ...
        sql = f"""
            SELECT
                d.first_name, d.last_name, d.specialization AS department,
                COUNT(d.doctor_id) AS number_of_doctors,
                ROUND(COALESCE(SUM(d.salary), 0)::NUMERIC, 2) AS total_salary
            FROM {self.industry}.doctors AS d
            LEFT JOIN {self.industry}.departments AS dp ON d.dept_id = dp.dept_id
